...
class ArpodCrtbp(gym.Env):
    ...
    def step(self, action):
        ...
        x_new = self.state[6:-1]
        self.state = np.append(self.state, self.max_time - self.time)

        # REWARD
        reward = self.get_reward(T, x_new, x_old)
    ...
    def get_reward(self, T, x_new, x_old):
        # Useful data
        x_norm_new = np.linalg.norm(
            np.array(
                [
                    x_new[0:3] * self.l_star / self.rho_max,
                    x_new[3:6] * self.l_star / (self.t_star * self.rhodot_max),
                ]
            )
        ...
        x_norm_old = np.linalg.norm(
            np.array(
                [
                    x_old[0:3] * self.l_star / self.rho_max,
                    x_old[3:6] * self.l_star / (self.t_star * self.rhodot_max),
                ]
            )
        ) / np.linalg.norm(np.array([1, 1, 1, 1, 1, 1]))
        ...
        reward = (np.linalg.norm(x_norm_old) - np.linalg.norm(x_norm_new))
