...
class ArpodCrtbp(gym.Env):
    ...
    def step(self, action):
        ...
        x0 = x0[0:-1]
        x_old = x0[6:-1]
        rho_old = np.linalg.norm(x0[6:9]) * self.l_star
    ...
    def get_reward(self, T, x_new, x_old):
        ...
        scale = np.concatenate([self.l_star * np.ones(3), self.l_star * np.ones(3) / self.t_star])

        reward = (np.linalg.norm(np.multiply(x_old, scale)) - np.linalg.norm(np.multiply(x_new, scale)))  # TODO: check dimensioni
